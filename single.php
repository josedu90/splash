	...
	<?php else: ?>
		...
		<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			<div class="stm-single-post stm-default-page <?php if(splash_is_layout('magazine_one')) echo 'stm-single-post-magazine'?>">
				<div class="container">
					<?php if ( have_posts() ) :
						$template = (splash_is_layout('magazine_one') || splash_is_layout('magazine_two') || splash_is_layout('soccer_news')) ? 'partials/global/post-content-magazine': 'partials/global/post-content';
						while ( have_posts() ) : the_post();
